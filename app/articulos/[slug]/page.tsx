...
export default function ArticuloPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  // Pick related articles (other than current), complete with fallback guías if needed
  const fromArticles = Object.values(articles)
    .filter((a) => a.slug !== slug)
    .map((a) => ({ slug: a.slug, tag: a.tag, title: a.title, date: a.date, href: `/articulos/${a.slug}` }))

  const related: { slug: string; tag: string; title: string; date: string; href: string }[] = [...fromArticles]
  if (related.length < 2) {
